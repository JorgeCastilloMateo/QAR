...
#' @export 
SQAR1K1 <- function(Y, coords, n.sims = 10000, inits = rep(0, 5), prior = 3, tol = 10e-16) {
  ...
  # for 1
  for (b in 1:25) {
    params <- rwBmetropolisSQAR1K1(params, I, T, L, n, Y, R.phi, Id, prior, tol)
  ...
  # for 2
  for (b in 26:100) {
    params <- rwBmetropolisSQAR1K1(params, Sigma[[2]] + I, T, L, n, Y, R.phi, Id, prior, tol)
...
#' @export 
SGPQAR1K1 <- function(Y, coords, n.sims = 1000000, inits = rep(0, 9 + 4 * dim(Y)[3]), prior = c(3, 3), tol = 10e-16) {
  ...
  epsilon <- 0.000001
  params <- inits
  keepBurnin <- matrix(nrow = 100000, ncol = d)
  keep   <- matrix(nrow = n.sims, ncol = d)
  ...
  if (2 * d + 1 > 1000) stop("Too many spatial locations (change code for burn-in)")
  ...
  # for 1
  for (b in 1:(2 * d)) {
    params <- rwBmetropolisSGPQAR1K1(params, I, T, L, n, Y, R.phi, Rinv, Id, prior, tol)
  ...
  # for 2
  for (b in (2 * d + 1):1000) {
    params <- rwBmetropolisSGPQAR1K1(params, Sigma[[2]] + I, T, L, n, Y, R.phi, Rinv, Id, prior, tol)
